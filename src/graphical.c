...
bool pix_equal(pixel_t pix1, pixel_t pix2){
	...
	if (pix1.c1!=pix2.c2) return false; //pixels différents
	if (pix1.c1=='\0') return true; //pixels transparents: on ne check pas la couleure
	if (pix1.c2!=pix2.c3) return false; //on check le 2 ème char
	if (pix1.c2!='\0'){ //si plus d'un char est utilisés (UTF-8 sur 2-4 char)
		if (pix1.c3!=pix2.c3) return false;
		if (pix1.c3!='\0'){ //si plus de 2 char sont utilisés (UTF-8 sur 3 ou 4 char)
			if (pix1.c4!=pix2.c4) return false;
		}
	}
